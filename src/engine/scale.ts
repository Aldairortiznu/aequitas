...
/**
 * Escalado del canvas: entero cuando cabe (2x, 3x, 4x) para píxel limpio;
 * fraccionario solo cuando la ventana es más pequeña que la resolución lógica (móviles).
 */
export function computeZoom(viewportWidth: number, viewportHeight: number): number {
  ...
  return raw >= 1 ? Math.floor(raw) : Math.max(0.25, Math.floor(raw * 100) / 100);
}
