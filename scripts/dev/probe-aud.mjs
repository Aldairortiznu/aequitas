...
await page.evaluate(async () => { const s = window.__aequitas.session; await s.applyNow([{ type: 'addEvidence', id: 'gym-acta-sin-firma' }, { type: 'addEvidence', id: 'gym-certificado-falso' }, { type: 'registrarVoz', id: 'gym-testimonio-eladio' }, { type: 'unlockCodice', id: 'cp-4' }, { type: 'unlockCodice', id: 'cp-14' }, { type: 'unlockCodice', id: 'cp-29' }]); void s.runActions([{ type: 'startAudiencia', id: 'gym-audiencia' }]); });
...
for (let i = 0; i < 8 && (await page.locator('.dlg').count()) > 0; i++) { await page.keyboard.press('Enter'); await page.waitForTimeout(300); }
await page.waitForTimeout(500);
const st = await page.evaluate(() => { const s = window.__aequitas.session.state; return { flags: s.flags, leg: s.legitimidad, notas: s.notas, used: s.codiceUsedIn }; });
console.log(JSON.stringify({ meters, fin, st, audOpen: await page.locator('.aud').count(), toast: await page.locator('.ui-toast').textContent().catch(() => null), errors }, null, 1));
