...
await page.evaluate(() => { const s = window.__aequitas.session; void s.runActions([{ type: 'startPacto', id: 'gym-pacto' }]); });
...
for (let i = 0; i < 6 && (await page.locator('.dlg').count()) > 0; i++) { await page.keyboard.press('Enter'); await page.waitForTimeout(300); }
const st = await page.evaluate(() => { const s = window.__aequitas.session.state; return { pactos: s.pactos, leg: s.legitimidad, flags: s.flags, mapStates: s.mapStates, dia: s.diaDeJuego }; });
